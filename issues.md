...
- [ ] **Integrate Profile settings button** with settings linked to the authenticated user, and settings saved in database. possible (and not obligated!) Settings: nickname, gender (male/female/non-binary)
- [ ] **Profile personalizations** like personalized fox avatars, ui color themes, bio text
- [x] **Refresh assigns a new room ID but partner bar stays visible (ghost session):** Fixed in `46230c0`-era branch — `sessionStorage` now persists the room ID. A refresh rejoins the same room instead of generating a new one. If the URL already has a `?room=` param (partner's shared link), it is written to `sessionStorage` on first load so subsequent refreshes stay in that room.
