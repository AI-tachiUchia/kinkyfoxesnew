  ...
  const steps = [
    {
      ...
      render: () => (
        <div className="flex flex-col gap-3">
          <button onClick={() => { setDistance('same-room'); setStep(1); }} className={`p-4 border rounded-xl transition-all text-left flex items-center gap-3 ${distance === 'same-room' ? 'bg-[#d97757]/20 border-[#d97757] text-white' : 'bg-[#181c22] border-white/10 hover:border-[#d97757]/50 text-gray-300'}`}>
            <span className="text-xl">🛋️</span>
            <span>{t.login.optSameRoom}</span>
          </button>
          <button onClick={() => { setDistance('tonight'); setStep(1); }} className={`p-4 border rounded-xl transition-all text-left flex items-center gap-3 ${distance === 'tonight' ? 'bg-[#d97757]/20 border-[#d97757] text-white' : 'bg-[#181c22] border-white/10 hover:border-[#d97757]/50 text-gray-300'}`}>
            <span className="text-xl">⏳</span>
            <span>{(t.login as any).optTonight}</span>
          </button>
          <button onClick={() => { setDistance('virtual'); setStep(1); }} className={`p-4 border rounded-xl transition-all text-left flex items-center gap-3 ${distance === 'virtual' ? 'bg-[#d97757]/20 border-[#d97757] text-white' : 'bg-[#181c22] border-white/10 hover:border-[#d97757]/50 text-gray-300'}`}>
            <span className="text-xl">📱</span>
            <span>{(t.login as any).optVirtual}</span>
          </button>
          <button onClick={() => { setDistance('custom'); setStep(1); }} className={`p-4 border rounded-xl transition-all text-left flex items-center gap-3 ${distance === 'custom' ? 'bg-[#d97757]/20 border-[#d97757] text-white' : 'bg-[#181c22] border-white/10 hover:border-[#d97757]/50 text-gray-300'}`}>
            <span className="text-xl">✏️</span>
            <span>{t.login.optCustom}</span>
          </button>
    ...
    {
      id: "heat",
      foxText: "Verstanden. Und wie intensiv soll es heute werden?",
      render: () => (
        <div className="flex flex-col gap-6 items-center pt-2">
          <div className="text-[#d97757] font-bold text-xl uppercase tracking-widest">{t.setup.heatLevels[heatLevel - 1]}</div>
          <input type="range" min={1} max={5} value={heatLevel} 
            onChange={e => setHeatLevel(Number(e.target.value))} 
            className="w-full heat-slider" />
          <button onClick={() => setStep(2)} className="w-full p-3 bg-[#d97757] hover:bg-[#e08568] text-[#121418] font-bold rounded-xl transition-all mt-4 shadow-[0_2px_10px_rgba(217,119,87,0.2)]">Weiter</button>
        </div>
      )
    },
    {
      id: "vibe",
      foxText: "Irgendeine bestimmte Fantasie oder Stimmung im Kopf?",
      render: () => (
    ...
    },
    {
      ...
      render: () => (
        <div className="flex flex-col gap-4">
          ...
          <button onClick={() => setStep(4)} className="w-full p-3 bg-[#d97757] hover:bg-[#e08568] text-[#121418] font-bold rounded-xl transition-all mt-4 shadow-[0_2px_10px_rgba(217,119,87,0.2)]">Fertig</button>
        </div>
      )
    },
    {
